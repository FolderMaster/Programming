...
namespace Programming.Model.Classes.Geometry
{
    public static class RectangleFactory
    {
        ...
        {
            double legth = Math.Round(_random.NextDouble() +
                _random.Next(0, widthCanvas - 31), 0);
            double width = Math.Round(_random.NextDouble() +
                _random.Next(0, heightCanvas - 31), 0);
            double x = Math.Round(_random.NextDouble() +
                _random.Next(15 + (int)legth / 2, widthCanvas - (16 + (int)legth / 2)), 0);
            double y = Math.Round(_random.NextDouble() +
                _random.Next(15 + (int)width / 2, heightCanvas - (16 + (int)width / 2)), 0);
