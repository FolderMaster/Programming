...
namespace ObjectOrientedPractics.Services
{
    ...
    public static class ValueValidator
    {
        ...
        /// <summary>
        /// Проверяет, что длина строки меньше максимального значения.
        /// </summary>
        /// <param name="value">Проверяемое значение.</param>
        /// <param name="maxLength">Максимальная длина строки.</param>
        /// <param name="name">Имя значения</param>
        /// <exception cref="ArgumentException"></exception>
        public static void AssertStringOnLessLength(string value, int maxLength, string name)
        {
            if (value.Length > maxLength)
            {
                throw new ArgumentException($"Length of {name} must be less than {maxLength}");
            }
        }
    }
}